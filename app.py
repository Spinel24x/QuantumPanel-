...
app = FastAPI(title="Quantum Panel v7")

def load_info():
    try: return json.loads(Path("/app/data/info.json").read_text())
    except: return {
        "uuid":"x","uuid_vmess":"x","trojan_pass":"x","ss_pass":"x","domain":"localhost",
        "vless":{"host":"localhost","port":443,"path":"/vless","sni":"www.speedtest.net"},
        "vmess":{"host":"localhost","port":443,"path":"/vmess","sni":"www.speedtest.net"},
        "trojan":{"host":"localhost","port":443,"path":"/trojan","sni":"www.speedtest.net"},
        "ss":{"host":"localhost","port":443,"path":"/ss","sni":"www.speedtest.net"}
    }
...
async def configs(
    ...
    if not address: address = "speed.cloudflare.com" if use_cdn else h
    
    security = "tls" if use_tls else "none"
    port = 443 if use_tls else 35093
    ...
    ss_link = f"ss://{base64.b64encode(f'aes-256-gcm:{i[\"ss_pass\"]}'.encode()).decode()}@{address}:{port}?path={i['ss']['path']}&host={h}#Quantum"
...
<html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><title>Quantum v7</title>
<style>
...
.checkbox-custom{width:20px;height:20px;border:2px solid rgba(102,0,204,0.5);border-radius:5px;display:inline-block;position:relative;transition:all 0.3s}
.checkbox-group input:checked+.checkbox-custom{background:#6600cc;border-color:#6600cc}
.checkbox-group input:checked+.checkbox-custom::after{content:'✓';position:absolute;top:-2px;left:4px;color:#fff;font-size:0.9em}
...
.address-area textarea:focus{outline:none;border-color:#6600cc;box-shadow:0 0 20px rgba(102,0,204,0.3)}
...
.input-row input:focus{outline:none;border-color:#6600cc;box-shadow:0 0 20px rgba(102,0,204,0.3)}
.btn{background:#6600cc;color:#fff;border:none;padding:14px 40px;border-radius:25px;cursor:pointer;font-family:'Courier New',monospace;font-size:0.85em;letter-spacing:3px;transition:all 0.3s}
.btn:hover{background:#9900ff;box-shadow:0 0 30px rgba(102,0,204,0.5);transform:scale(1.05)}
.info-badge{text-align:center;margin:15px 0;padding:10px;border-radius:20px;background:rgba(102,0,204,0.15);border:1px solid rgba(102,0,204,0.2);font-size:0.7em;color:rgba(255,255,255,0.5);letter-spacing:2px;display:none}
.info-badge.show{display:block}
.tabs{display:flex;justify-content:center;gap:10px;margin-bottom:25px;flex-wrap:wrap}
.tab{padding:15px 25px;background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.08);border-radius:30px;cursor:pointer;color:rgba(255,255,255,0.5);transition:all 0.3s;backdrop-filter:blur(5px)}
.tab:hover{background:rgba(102,0,204,0.2);color:#fff;border-color:rgba(102,0,204,0.4)}
...
.config-item.active{display:block;animation:fadeIn 0.3s ease}
...
.copy-btn{position:absolute;top:10px;right:10px;background:#6600cc;color:#fff;border:none;padding:6px 14px;border-radius:15px;cursor:pointer;font-size:0.65em;letter-spacing:1px}
...
<label class="checkbox-group" onclick="toggleTLS()">
<input type="checkbox" id="tlsCheck" checked><span class="checkbox-custom"></span>TLS
</label>
<label class="checkbox-group" onclick="toggleCDN()">
<input type="checkbox" id="cdnCheck"><span class="checkbox-custom"></span>Cloudflare CDN
</label>
</div>

<div class="input-row">
<input id="sniInput" value="www.speedtest.net" placeholder="SNI">
</div>
...
104.26.0.1
104.26.1.1
... (هر خط = یک آدرس)"></textarea>
<p class="hint">✦ هر خط یک IP/Address ✦</p>
</div>
...
draw(ctx){const alpha=Math.sin(Math.PI*this.life/this.maxLife)*0.4;ctx.strokeStyle=`rgba(${this.color[0]},${this.color[1]},${this.color[2]},${alpha})`;ctx.lineWidth=1.2;ctx.shadowBlur=8;ctx.shadowColor=`rgba(${this.color[0]},${this.color[1]},${this.color[2]},${alpha})`;ctx.beginPath();
...
for(let i=0;i<25;i++)particles.push(new DNA());
(function a(){ctx.fillStyle='rgba(0,0,0,0.08)';ctx.fillRect(0,0,c.width,c.height);particles.forEach(p=>{p.update();p.draw(ctx)});requestAnimationFrame(a)})();

function toggleTLS(){
const cb=document.getElementById('tlsCheck');cb.checked=!cb.checked;
document.getElementById('sniInput').style.display=cb.checked?'inline-block':'none';
}
function toggleCDN(){
const cb=document.getElementById('cdnCheck');cb.checked=!cb.checked;
document.getElementById('addressArea').classList.toggle('show',cb.checked);
}

let configs={};
async function gen(){
const tls=document.getElementById('tlsCheck').checked?'1':'0';
const cdn=document.getElementById('cdnCheck').checked?'1':'0';
const sni=document.getElementById('sniInput').value||'www.speedtest.net';
let addr='';
if(cdn==='1'){addr=document.getElementById('addressInput').value.split('\\n')[0].trim()||'speed.cloudflare.com'}
const r=await fetch(`/api/configs?address=${addr}&sni=${sni}&tls=${tls}&cdn=${cdn}`);
configs=await r.json();
const s=configs.settings;
document.getElementById('infoBadge').textContent=`✦ ${s.host} ✦ :${s.port} ✦ TLS:${s.tls?'ON':'OFF'} ✦ CDN:${s.cdn?'ON':'OFF'}`;
document.getElementById('infoBadge').classList.add('show');
render();
}
...
const t=document.createElement('div');t.className='tab'+(first?' active':'');t.innerHTML=`<span class="icon">${v.icon}</span><span class="name">${v.name}</span>`;t.onclick=(e)=>{document.querySelectorAll('.tab').forEach(x=>x.classList.remove('active'));document.querySelectorAll('.config-item').forEach(b=>b.classList.remove('active'));t.classList.add('active');document.getElementById('item-'+k).classList.add('active')};tc.appendChild(t);
const b=document.createElement('div');b.className='config-item'+(first?' active':'');b.id='item-'+k;
b.innerHTML=`<div class="config-icon">${v.icon}</div><div class="config-name">${v.name}</div><div class="config-value"><button class="copy-btn" onclick="copy('${k}',this)">COPY</button>${v.link}</div>`;bc.appendChild(b);first=false}
}

function copy(k,btn){if(configs[k]){navigator.clipboard.writeText(configs[k].link);btn.textContent='✓';btn.style.background='#0f0';btn.style.color='#000';setTimeout(()=>{btn.textContent='COPY';btn.style.background='#6600cc';btn.style.color='#fff'},2000)}}
setTimeout(gen,300);
</script></body></html>""")
