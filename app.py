...
PersistentKeepalive = 25"""
    ...
    return JSONResponse({
        "vless": {"name":"VLESS","icon":"🟣","link":f"vless://{i['uuid']}@{i['vless']['host']}:{i['vless']['port']}?encryption=none&security=none&type=ws&path={i['vless']['path']}&host={h}#Quantum"},
        "vmess": {"name":"VMess","icon":"🟠","link":vmess_link},
        "trojan": {"name":"Trojan","icon":"🔴","link":f"trojan://{i['trojan_pass']}@{i['trojan']['host']}:{i['trojan']['port']}?security=none&type=ws&path={i['trojan']['path']}&host={h}#Quantum"},
        "wireguard": {"name":"WireGuard","icon":"🟢","config":wg_config},
        "ssh": {"name":"SSH","icon":"🔵","command":f"ssh -D 1080 -p {i['ssh']['port']} root@{i['ssh']['host']}","pass":i['ssh']['pass']}
    })
...
.container{position:relative;z-index:2;max-width:800px;margin:0 auto;padding:20px}
...
.subtitle{text-align:center;color:rgba(255,255,255,0.4);margin-bottom:40px;font-size:0.75em;letter-spacing:5px}
.tabs{display:flex;justify-content:center;gap:15px;margin-bottom:30px;flex-wrap:wrap}
.tab{padding:15px 25px;background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.08);border-radius:30px;text-align:center;cursor:pointer;color:rgba(255,255,255,0.5);transition:all 0.3s;backdrop-filter:blur(5px)}
...
.config-area{margin-top:20px;text-align:center}
.config-item{display:none;padding:20px;margin:10px 0;border-radius:15px;background:rgba(255,255,255,0.02);border:1px solid rgba(255,255,255,0.05);backdrop-filter:blur(10px)}
...
.config-value{color:#00ff41;font-size:0.7em;word-break:break-all;line-height:2;padding:15px;border-radius:10px;background:rgba(0,0,0,0.4);text-align:left;position:relative}
.copy-btn{position:absolute;top:10px;right:10px;background:#6600cc;color:#fff;border:none;padding:6px 14px;border-radius:15px;cursor:pointer;font-family:'Courier New',monospace;font-size:0.65em;letter-spacing:1px}
.copy-btn:hover{background:#9900ff}
.footer{text-align:center;margin-top:50px;color:rgba(255,255,255,0.15);font-size:0.6em;letter-spacing:5px}
</style></head>
<body><canvas id="c"></canvas>
<div class="container">
<h1 class="title">QUANTUM</h1>
<p class="subtitle">✦ SECURE TUNNEL PROTOCOLS ✦</p>
<div class="tabs" id="tabs"></div>
<div class="config-area" id="boxes"></div>
<p class="footer">VLESS  ✦  VMess  ✦  Trojan  ✦  WireGuard  ✦  SSH</p>
</div>
<script>
...
const b=document.createElement('div');b.className='config-item'+(first?' active':'');b.id='item-'+k;let txt=v.link||v.config||v.command||'';
